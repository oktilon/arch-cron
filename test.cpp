...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    for(int i = 0; i < argc; i++) {
        ...
        printf("Usage %s%s%s [%s-d%s %sdev1 dev2 ...%s]\n"
               "  where : %s-d%s, %s--devices%s    - copy devices only from list\n"
    ...
    printf("Test Arch pid=%d, dev=%s%s%s\n",
                pid,
                bd, buff, Copy::col_e);
    ...
    for(int ix=0; ix < devices_count; ix++) {
        ...
        try {
            ...
            printf("Test DEV=%s%d%s [%s]", Copy::col_y, id, Copy::col_e, inf.sBeg);
